...
class jqGridArray
{
	...
	private function parse_query()
	{
		...
		//$this->parse_query 			= preg_split('#(/\s+SELECT\s+/is|/\s+COUNT\s+/is|(\s)DISTINCT(\s)|(\s)FROM(\s)|JOIN|WHERE|ORDER(\s+)BY|(\s)LIMIT(\s)|(\s)OFFSET(\s))+#i', $this->parse_query, -1, PREG_SPLIT_DELIM_CAPTURE);
		$words = array('SELECT ', ' DISTINCT ',' FROM ',' JOIN ',' WHERE ',' ORDER BY ',' LIMIT ',' OFFSET ');
		$ad = array();
	...
	/* Get next non empty array element 
	 * 
	 */
