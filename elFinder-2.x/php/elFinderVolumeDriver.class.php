	...
	public function mount(array $opts) {
		...
		if ($root['read']) {
			...
			if ($this->options['startPath']) {
				...
				//$firephp->log($start, 'Iterators');
				//$testvar = $this->_inpath($this->options['startPath'], $this->root);
				$testvar = strpos($this->options['startPath'], ($this->root).DIRECTORY_SEPARATOR) == 0;
				//$firephp->log($testvar,'DEBUG_INPATH_TEST');
				//$firephp->log($this->root,'Parent');
				//$firephp->log($this->options['startPath'], 'Path');
				//$firephp->log(empty($start['hidden']), 'DEBUG EMPTY START HIDDEN');
				if (!empty($start)
