...
if ((isset($_COOKIE['username'])) && (!isset($_GET['logout']))) {
	header('Location: http://10.10.10.199/staging/index.php');
}
...
if (strpos(strtoupper($_POST["username"]),'.')){
$domain = "@EXTRANET.FINLEY-COOK.COM";
}
else {$domain = "@FINLEY-COOK.COM";
}

$loginName = strtoupper($_POST["username"].$domain);
...
if ($_POST["oldform"]) {
	...
	if($loginName != NULL && $password != NULL){
		...
		if ($adldap->authenticate($loginName, $password)){
			...
			$redir0 = "Location: https://" . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . "/condition.php";
...
<div class="box-content">
	<form action='<?php echo $_SERVER["PHP_SELF"]; ?>' method="post" id="Cred_Form" accept-charset=utf-8 name="login">
        ...
        <div class="login">
        Username: <input class="formField" name="username" type="text" id="username">
        </div>
        <div class="login">
        Password: <input class="formField" name="password" type="password" id="password">
        </div>
        <!--Domain: <select name="domain">
        	<option value="@extranet.finley-cook.com">CLIENT</option>
            <option value="@finley-cook.com">EMPLOYEE</option>
            </select>-->
        <div class="login">
		<input class="button" name="submit" type="submit" id="submit" value="Submit">
        </div>
